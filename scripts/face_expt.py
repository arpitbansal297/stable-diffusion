...
def main():
    ...
    for batch_ind, image in enumerate(trainloader):
        xc = image.shape[0] * ["A headshot of person"]
        cond = model.module.get_learned_conditioning(xc)

        og_img = next(dl).cuda()
        temp = (og_img + 1) * 0.5
        utils.save_image(temp, f'{results_folder}/og_img_{batch_ind}.png')

        with torch.no_grad():
            og_img_guide, og_img_mask = operation.operation_func(og_img, return_faces=True, mtcnn_face=True)

        # image is just for dimensions
        output = model.module.operation_diffusion(og_img=image, operated_image=og_img_guide, cond=cond, operation=operation)

        for i in range(image.shape[0]):
            img_ = return_cv2(output[i], f'{results_folder}/out_img_{cnt}.png')
            img_ = return_cv2(og_img[i], f'{results_folder}/og_img_{cnt}.png')
            cnt+=1
